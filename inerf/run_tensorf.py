...
def run():
    ...
    kwargs = ckpt['kwargs']
    kwargs.update({'device':device, 'near_far': [near, far]})
    # tensorf = eval(args.tensorf_model_name)(**kwargs)
    # tensorf = TensorVMSplit(**kwargs)
    tensorf = TensorCP(**kwargs)
    tensorf.load(ckpt)
    ...
    for k in range(1000):
        ...
        if sampling_strategy == 'random':
            ...
            batch = np.random.randint(0, H*W, batch_size)
        ...
        target_s = obs_img_noised.reshape(H*W, -1)[batch]
        target_s = torch.Tensor(target_s).to(device)
        pose = cam_transf(start_pose)

        rays_o2, rays_d2 = get_rays(H, W, focal, pose)  # (H, W, 3), (H, W, 3)
        # rays_o2 = rays_o2[batch[:, 1], batch[:, 0]]  # (N_rand, 3)
        # rays_d2 = rays_d2[batch[:, 1], batch[:, 0]]
        # batch_rays = torch.stack([rays_o, rays_d], 0)

        ## Required for TensoRF
        directions = trf_ru.get_ray_directions_blender(H, W, [focal,focal]).cuda()  # (h, w, 3)
        # directions = trf_ru.get_ray_directions(H, W, [focal,focal]).cuda()  # (h, w, 3)
        directions = directions / torch.norm(directions, dim=-1, keepdim=True)
        rays_o, rays_d = trf_ru.get_rays(directions, pose)
        # Select subset of rays
        rays_o = rays_o[batch]  # (N_rand, 3)
        rays_d = rays_d[batch]
        batch_rays = torch.cat([rays_o, rays_d], 1)
        batch_rays = batch_rays.view(-1,batch_rays.shape[-1])
        
        # trf.ray_utils.get_rays()
        # batch_rays = torch.hstack([rays_o, rays_d])

        # rgb, disp, acc, extras = render(H, W, focal, chunk=args.chunk, rays=batch_rays,
        #                                 verbose=k < 10, retraw=True,
        #                                 **render_kwargs)
        rgb, _, depth, _, _ = OctreeRender_trilinear_fast(batch_rays, tensorf, device=device)
        ...
        if (k + 1) % 20 == 0 or k == 0:
            ...
            if OVERLAY is True:
                with torch.no_grad():
                    # rays = torch.stack(get_rays(H, W, focal, c2w=pose[:3, :4]), 0)
                    rays_o2, rays_d2 = get_rays(H, W, focal, c2w=pose[:3, :4])
                    ...
                    rgb, _, depth, _, _ = OctreeRender_trilinear_fast(rays, tensorf, device=device)
                    rgb = rgb.cpu().detach().numpy().reshape(H, W, 3)
                    rgb8 = to8b(rgb)
                    ref = to8b(obs_img)
